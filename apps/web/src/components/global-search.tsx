    ...
    <>
      <Tooltip>
        <TooltipTrigger asChild>
            <Button variant="outline" className="w-full max-w-xs" onClick={() => setIsModalOpen(true)} data-testid="global-search-trigger">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="top-1/4" onInteractOutside={closeModal} data-testid="global-search-dialog">
          ...
          <div className="mt-4 min-h-[200px]" data-testid="global-search-results">
            {results && !isLoading && (
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      {results.clusters.map(cluster => (
                        <Link key={cluster.id} href={`/clusters/${cluster.id}`} onClick={closeModal} className="flex items-center gap-3 p-2 rounded-md hover:bg-accent" data-testid={`search-result-cluster-${cluster.id}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      {results.services.map(service => (
                        <Link key={service.id} href={`/services/${service.id}`} onClick={closeModal} className="flex items-center gap-3 p-2 rounded-md hover:bg-accent" data-testid={`search-result-service-${service.id}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      {results.hosts.map(host => (
                        <Link key={host.id} href={`/hosts/${host.id}`} onClick={closeModal} className="flex items-center gap-3 p-2 rounded-md hover:bg-accent" data-testid={`search-result-host-${host.id}`}>
            ...
            {!results && !isLoading && query.length < 2 && (
                 <div className="text-center py-8 text-muted-foreground" data-testid="search-prompt">
